...
export default function EditProductPage({ product: initialProduct, categories }: EditProductPageProps) {
  ...
  const [existingImages, setExistingImages] = useState<ProductImage[]>(initialProduct.images)
  ...
  const handleNameChange = (value: string) => {
    ...
    if (!slug || slug === initialProduct.slug) {
      setSlug(
        value
          .toLowerCase()
          .trim()
  ...
  // Reorder existing images (move up)
  ...
  // Reorder existing images (move down)
  ...
  const handleSave = async () => {
    ...
    try {
      // Combine all images (existing + new) in order
      const allImageUrls = [
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-8 space-y-6">
        ...
        <div>
          <label
            ...
            style={{ fontFamily: 'var(--font-raleway)' }}
          >
            Images (max: 6) {totalImages > 0 && <span className="text-gray-500">({totalImages}/6)</span>}
          </label>
          ...
          <div className="grid grid-cols-3 gap-4">
              ...
              <div key={img.id} className="relative group">
                ...
                <div className="absolute bottom-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  {i < existingImages.length - 1 && (
                    <button
                      type="button"
                      onClick={() => moveExistingImageDown(i)}
                      className="bg-black/70 text-white px-2 py-1 text-xs rounded"
            ...
            {newImages.map((url, i) => (
              <div key={`new-${i}`} className="relative group">
                ...
                {/* Reorder buttons */}
                <div className="absolute bottom-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {existingImages.length + i > 0 && (
                    <button
                      type="button"
                      onClick={() => moveNewImageUp(i)}
                      className="bg-black/70 text-white px-2 py-1 text-xs rounded"
                      title="Déplacer vers le haut"
                    >
                      ↑
                ...
                {/* New badge */}
                <div className="absolute bottom-2 right-2 bg-green-600 text-white px-2 py-1 text-xs rounded">
                  Nouveau
                </div>
              </div>
            ))}
        ...
        <div className="flex items-center justify-between pt-6 border-t border-gray-200">
          <button
            ...
            disabled={isDeleting}
            className="px-6 py-2.5 bg-red-600 text-white uppercase tracking-[0.2em] text-xs hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed rounded-lg"
            style={{ fontFamily: 'var(--font-raleway)' }}
          >
